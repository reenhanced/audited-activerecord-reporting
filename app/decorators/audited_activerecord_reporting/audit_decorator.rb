class AuditedActiverecordReporting::AuditDecorator < Draper::Decorator
  decorates 'AuditedActiverecordReporting::Audit'
  delegate_all
  ...
  def changes
    ...
    leading_content = h.content_tag :div, class: 'lead audit-change' do
      h.content_tag(:span, action, class: "label #{h.bootstrap_class(object.action, type: 'label')}") +
      name
    end

    audited_content = h.div_for object, class: 'collapse' do
  ...
  def author
    h.content_tag :strong, class: 'text-info' do
      ...
      h.content_tag(:br) +
      h.content_tag(:em, AuditedActiverecordReporting::Audit.human_attribute_name(:created_at, datetime: object.created_at))
    end
  end
  ...
  protected
  def field_name(field)
    (object.revision || object.auditable).class.human_attribute_name(field).downcase
  end

  private
  def presenter
    begin
      @presenter ||= "AuditedActiverecordReporting::AuditPresenter::#{object.auditable_type}".constantize.new(object, self)
    rescue NameError => e
      @presenter = AuditedActiverecordReporting::AuditPresenter::Base.new(object, self)
    end
  end
end
